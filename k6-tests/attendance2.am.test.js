...
function buildArchetype(archetype) {
  ...
  if (archetype === "perfect") {
    return { id: "perfect_am_only", actions: [amIn, amOut] };
  }
  if (archetype === "late") {
    const lateAmIn = { kind: "in", at: "09:30 AM" };
    const lateAmOut = { kind: "out", at: "12:50 PM" };
    return { id: "late_am_only", actions: [lateAmIn, lateAmOut] };
  }
  if (archetype === "no_time_in") {
    return { id: "no_time_in_out_only_am", actions: [amOut] };
  }
  if (archetype === "no_time_out") {
    return { id: "no_time_out_in_only_am", actions: [amIn] };
  }
