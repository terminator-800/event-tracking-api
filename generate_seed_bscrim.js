...
// Read CSV file
const csvContent = fs.readFileSync('BSCRIM.csv', 'utf-8');
...
// Start building SQL
let sql = `-- ============================================================
-- BSCRIM Complete Seed Data Script - ALL ${records.length} RECORDS
-- MySQL Workbench
...
records.forEach((record, i) => {
  ...
  let email = record['Email'] ? record['Email'].trim() : `noemail_${i}@bscrim.edu`;
  
  if (!email) email = `noemail_${i}@bscrim.edu`;
...
records.forEach((record, i) => {
  ...
  const comma = i < records.length - 1 ? ',' : ';';
  sql += `\n('${enrollmentRef}', (SELECT id FROM students WHERE student_id = '${studentId}'), (SELECT id FROM programs WHERE course_code = 'BSCRIM'), '${schoolYear}', '2nd sem', ${yearLevel})${comma}`;
});
...
// Write to file
fs.writeFileSync('seeds/bscrim_complete_seed_all.sql', sql, 'utf-8');

console.log(`✅ Complete seed script generated!`);
console.log(`📊 Total records: ${records.length} students`);
console.log(`📁 File: seeds/bscrim_complete_seed_all.sql`);
const fileSize = fs.statSync('seeds/bscrim_complete_seed_all.sql').size / 1024 / 1024;
