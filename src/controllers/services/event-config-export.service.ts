...
export async function importEventConfig(opts: {
  ...
  const { hash: passwordHash, encrypted: passwordEncrypted } =
    await prepareEventPasswordStorage(password);
  ...
  try {
    ...
    const [result] = await connection.execute<ResultSetHeader>(
      `INSERT INTO events (
        ...
        status, audience_notes, fine_amount, attendance_password_hash, attendance_password_encrypted,
